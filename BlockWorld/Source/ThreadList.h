...
namespace utils
{
	...
	public:
		...
		void pushTask(std::function<void(void)> task)
		{
			...
			workLoad.emplace(std::move(task));
	...
	private:
		...
		std::queue<std::function<void(void)>> workLoad;
	private:
			...
			{
				{
					...
					conditional.wait(collectTaskLock, [this] {
						return !workLoad.empty() || terminating;
						});

					// will not exit unless destruction is called and all work has completed.
					if (workLoad.empty() && terminating)
					{
						BW_DEBUG("<Thread %d closed.>", id);
						return;
					}

					current_task = std::move(workLoad.front());
					workLoad.pop();
				}
