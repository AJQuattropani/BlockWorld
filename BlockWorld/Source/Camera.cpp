...
void bwrenderer::Camera::updateContext()
{
    if (updateFlags & UPDATE_PROJECTION_FLAG) outputContext->projectionMatrix =
        glm::perspective(glm::radians(fov), static_cast<float>(outputContext->screen_width_px) / outputContext->screen_height_px, 0.1f, outputContext->ch_render_unload_distance * 15.0f);
